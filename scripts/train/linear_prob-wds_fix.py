...
def get_args_parser():
    ...
    parser.add_argument(
        "--data_path",
        # use "/projectnb/dl4ds/materials/datasets/imagenet-tokenized"
    ...
    parser.add_argument(
        "--train_pattern",
        default="imagenet-train-{000000..000319}.tar",
        type=str,
        help="WebDataset train pattern",
    )
    parser.add_argument(
        "--val_pattern",
        default="imagenet-val-{000000..000049}.tar",
        type=str,
        help="WebDataset val pattern",
    )
    parser.add_argument(
        "--nb_classes",
        default=1000,
...
def main(args):
    ...
    # Calculate number of samples for epoch size tracking
    # Imagenet typically has ~1.28M training samples
    NUM_IMAGENET_TRAIN_SAMPLES = 1_281_167
    NUM_IMAGENET_VAL_SAMPLES = 50_000
    ...
    train_pattern = os.path.join(args.data_path, args.train_pattern)
    val_pattern = os.path.join(args.data_path, args.val_pattern)
    ...
    dataset_val = wds.WebDataset(
        val_pattern,
        shardshuffle=False,
        nodesplitter=wds.split_by_node if (num_tasks > 1 and args.dist_eval) else None,
    )
    dataset_val = dataset_val.decode("pil").map(transform_val_sample)
    dataset_val = dataset_val.batched(args.batch_size)
    ...
    ONE_EPOCH_TRAIN = math.ceil(NUM_IMAGENET_TRAIN_SAMPLES / GLOBAL_BATCH_SIZE)
    ONE_EPOCH_VAL = math.ceil(NUM_IMAGENET_VAL_SAMPLES / GLOBAL_BATCH_SIZE)
    ...
    if args.eval:
        ...
        print(
            f"Accuracy of the network on the validation images: {test_stats['acc1']:.1f}%"
        )
        exit(0)
