...
from efficient_cv.models.layers.base_model import BaseModel
from efficient_cv.models.layers.blocks import UViTBlock


class ImageBert(
    BaseModel,
    PyTorchModelHubMixin,
    tags=["arxiv:2406.07550", "image-generation"],
    ...
    def __init__(self, config):
        ...
        self.model.lm_head = nn.Linear(
            self.hidden_size, self.target_codebook_size, bias=True
        )
    ...
    def generate(
        ...
        ids = torch.full(
            (condition.shape[0], self.image_seq_len), self.mask_token_id, device=device
        )

        cfg_scale = guidance_scale if guidance_decay == "constant" else 0.0
        ...
        for step in range(num_sample_steps):
            ...
            if guidance_decay == "power-cosine":
                ...
                scale_step = (
                    (
                        1
                        - torch.cos(
                            ((step / num_sample_steps) ** guidance_scale_pow) * torch.pi
                        )
                    )
                    * 1
                    / 2
            ...
            if cfg_scale != 0:
                cond_logits = self.forward(ids, condition, cond_drop_prob=0.0)
                uncond_logits = self.forward(ids, condition, cond_drop_prob=1.0)
                if guidance_decay == "power-cosine":
                    logits = uncond_logits + (cond_logits - uncond_logits) * cfg_scale
                else:
                    logits = cond_logits + (cond_logits - uncond_logits) * cfg_scale
            else:
                logits = self.forward(ids, condition, cond_drop_prob=0.0)
            ...
            mask_len = torch.Tensor([np.floor(self.image_seq_len * mask_ratio)]).to(
                device
            )
...
class UViTBert(ImageBert):
    def __init__(self, config):
        ...
        self.embeddings = nn.Embedding(
            self.target_codebook_size + self.condition_num_classes + 2, self.hidden_size
        )

        self.pos_embed = nn.init.trunc_normal_(
            nn.Parameter(
                torch.zeros(
                    1, self.config.model.generator.image_seq_len + 1, self.hidden_size
                )
            ),
            0.0,
            0.02,
